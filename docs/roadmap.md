...
- **Reading tracking**: record reading goals/logs (per child or family member) — pages/books read, current book, progress toward a goal. Fits the Day Dashboard (e.g. a "Reading" module alongside Kids' Schedule) and the existing per-user `memberType` model. Needs a `reading_logs` table (userId/familyId, book, pages, date) and a progress/goal model.
