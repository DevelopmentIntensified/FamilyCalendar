<script lang="ts">
	...
	export let onChangeInviteType: ((value: string, type: 'required' | 'optional') => void) | null =
		null;
		...
		<div class="mb-2 flex flex-wrap gap-2">
			{#each selected as att}
				...
				{@const displayName = member
					...
					: att}
				<span
					class="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-2 py-1 pr-3 text-sm"
				>
					<span
					...
					{#if member && onChangeInviteType}
						<button
							type="button"
							on:click={() =>
								onChangeInviteType(att, selections[att] === 'required' ? 'optional' : 'required')}
							class="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold transition-colors {selections[
								att
							] === 'required'
					...
					<button
						type="button"
						on:click={() => pick(att)}
						class="-m-1.5 rounded-full p-1.5 text-slate-400 hover:text-slate-600"
			...
			>
					...
					<div class="p-1">
						{#each filteredFamilyMembers as member}
							...
							<button
								...
								class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors hover:bg-slate-50 {isSelected
									? 'bg-primary-50'
									: ''}"
							>
								<div
									class="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-semibold"
									style="background-color: {color.bg}; color: {color.text}"
								>
									{member.firstName?.charAt(0)}{member.lastName?.charAt(0)}
									{#if isSelected}
										<div
											class="absolute -bottom-0.5 -right-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-emerald-500 ring-2 ring-white"
										>
											<svg
				...
				{#if filteredRecent.length > 0}
					<div class="border-t border-slate-100 p-1">
						<div
							class="px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400"
						>
							Recent
						</div>
						{#each filteredRecent as att}
							...
							<button
								type="button"
								on:click={() => pick(att)}
								class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors hover:bg-slate-50 {isSelected
									? 'bg-primary-50'
									: ''}"
							>
								<span
				...
				{#if contactSearch.trim() && !filteredFamilyMembers.length && !filteredRecent.length}
					<button
						type="button"
						on:click={addCustom}
						class="m-1 w-full rounded-lg border-2 border-dashed border-slate-200 py-2 text-sm text-slate-500 transition-all hover:border-primary-300 hover:bg-primary-50 hover:text-primary-700"
					>
