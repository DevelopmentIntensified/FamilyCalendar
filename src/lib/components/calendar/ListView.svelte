		...
		.filter((event) => {
			...
			const eventDate =
				event.date instanceof Date
					...
					: DateTime.fromISO(String(event.date));
	...
	$: monthTasks = dueTasks
		...
		.filter((t) => {
			const d = DateTime.fromJSDate(
				t.dueDate instanceof Date ? t.dueDate : new Date(String(t.dueDate))
			);
...
<div class="space-y-6">
	{#if filteredEvents.length === 0 && monthTasks.length === 0}
		<div class="flex flex-col items-center justify-center py-12 text-center">
			<img src={todoList} alt="" class="mx-auto mb-4 h-24 w-auto opacity-80" />
			<p class="max-w-xs text-lg font-medium text-slate-700">
				A whole month of nothing. Suspicious. Add something fun?
			</p>
		</div>
	{/if}
	...
	{#each allDates as date}
		...
		<div>
			<h3
				class="sticky top-0 z-10 mb-3 bg-white py-2 text-sm font-semibold text-slate-500 {isToday
					? 'text-primary-600'
					: ''}"
			>
						...
						>
							...
							<div class="min-w-0 flex-1">
								<div class="flex items-center gap-2">
									<h4 class="truncate font-medium text-slate-900 group-hover:text-primary-600">
										{event.title}
									</h4>
									{#if rv}
										<span
								...
								<div class="mt-1 flex flex-wrap items-center gap-x-3 text-sm text-slate-500">
									...
									{:else}
										...
										{@const et =
											event.endTime || (event.end ? formatEventTime(event.end) : undefined)}
										{#if st}
											<span class="flex items-center gap-1">
												<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
													<path
							...
							<svg
								class="h-5 w-5 shrink-0 text-slate-400 group-hover:text-slate-600"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
							>
