				...
				<div>
					...
					{#if family}
						<p class="mt-1 text-sm text-slate-500">
							Managing invitations for: <span class="font-medium text-slate-700">{family.name}</span
							>
						</p>
					{/if}
				</div>
							...
							<div class="flex flex-wrap items-start justify-between gap-4">
								<div class="min-w-0 flex-1">
									<div class="mb-2 flex flex-wrap items-center gap-2">
										<code class="rounded bg-slate-100 px-2 py-0.5 font-mono text-sm text-slate-700"
											>{invite.code}</code
										>
										{#if !isActive}
											<span
