...
<div class="mx-auto max-w-6xl p-4 print:p-0">
	...
	<div class="mb-5 flex flex-wrap items-center justify-between gap-3 print:hidden">
		...
		<div class="flex items-center gap-2">
			<a
				...
				>← {new Date(data.prev.year, data.prev.month - 1).toLocaleString('en', {
					month: 'short'
				})}</a
			>
	...
	<!-- Fridge sheet -->
	<div class="fridge-sheet rounded-xl bg-white p-4 shadow-sm print:rounded-none print:shadow-none">
		...
		<div
			class="grid grid-cols-7 gap-px overflow-hidden rounded-lg border border-slate-300 bg-slate-300 print:border-slate-400"
		>
			{#each labels as label}
				<div
					class="bg-slate-100 py-1.5 text-center text-[11px] font-bold uppercase tracking-wider text-slate-500 print:bg-slate-200"
				>
					{label}
				</div>
			{/each}
			...
			{#each data.grid as cell (cell.iso)}
				<div
					class="cell bg-white p-1 {cell.inMonth ? '' : 'bg-slate-50/70'} {cell.isToday
				...
				>
					<div class="mb-0.5 flex items-center justify-between px-0.5">
						...
						{#if cell.items.length > 3}
							<span class="pr-0.5 text-[9px] font-semibold text-slate-400"
								>+{cell.items.length - 3}</span
							>
						{/if}
					</div>
						...
						{#each cell.items.slice(0, 3) as item}
							<li class="flex items-start gap-1 leading-tight">
								<span
									...
									style="background-color: {item.color}; -webkit-print-color-adjust: exact; print-color-adjust: exact;"
								></span>
								<span
