					...
					<div class="space-y-3">
						...
						<div class="flex items-center">
							...
							<span class="text-gray-700">
								{DateTime.fromJSDate(event.start)
									.setZone(timeZone)
									.toLocaleString(DateTime.TIME_SIMPLE)} - {DateTime.fromJSDate(event.end)
									.setZone(timeZone)
						...
						{#if calendar}
							<div class="flex items-center">
								...
								<span class="text-gray-700"
									>{isFamilyEvent ? 'Family Calendar' : 'My Calendar'}</span
								>
							</div>
						{/if}
						...
						<div class="flex items-center">
							...
							<span class="text-gray-700"
								>Created {DateTime.fromJSDate(event.created_at).toLocaleString(
									DateTime.DATE_MED
								)}</span
							>
						</div>
							...
							<div class="space-y-2">
								{#each attendees as attendee (attendee.id)}
									<div class="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2">
										...
										<span
											class="rounded-full px-2 py-1 text-xs font-medium {attendee.status === 'going'
												? 'bg-green-100 text-green-800'
												: attendee.status === 'maybe'
													? 'bg-yellow-100 text-yellow-800'
													: attendee.status === 'not_going' || attendee.status === 'declined'
				...
				{#if isFamilyEvent}
					<div class="mb-6 border-t pt-6">
						...
						<div
							...
							class="flex overflow-hidden rounded-xl border border-gray-200 bg-gray-50 p-1"
						>
							{#each RSVP_OPTIONS as option (option.status)}
								...
								<button
									...
									aria-pressed={active}
									class="flex flex-1 items-center justify-center gap-1.5 rounded-lg px-2 py-2.5 text-sm font-medium transition-all disabled:opacity-60 {option.status ===
									'going'
										? active
