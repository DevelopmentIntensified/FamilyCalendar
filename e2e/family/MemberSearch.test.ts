...
const searchUserEmail = 'delivered+search' + Date.now() + '@resend.dev';
let ownerId = '';
let searchUserId = '';
let familyId = '';
...
test('Member search and selection', async ({ page }) => {
	...
	await test.step('Search for user', async () => {
		await page.fill('input[placeholder="Type to search..."]', 'Search');
		await page.waitForTimeout(500);
	});

	await test.step('Verify search results appear', async () => {
		await expect(page.locator('ul >> text=Search User')).toBeVisible({ timeout: 5000 });
	});

	await test.step('Select user', async () => {
		await page.click('text=Search User');
	});

	await test.step('Verify user selected', async () => {
		await expect(page.locator('text=Search User')).toBeVisible();
		await expect(page.locator('button:has-text("Add to Family")')).toBeVisible();
	});

	await test.step('Add user to family', async () => {
		await page.click('button:has-text("Add to Family")');
