...
const email = 'quickcreate' + Date.now() + '@familyplanz.com';
...
test.beforeEach(async () => {
	...
	const uniqueEmail = 'quickcreate' + Date.now() + Math.random().toString(36).substring(2, 6) + '@familyplanz.com';
	let user = await createNewUser(firstName, lastName, uniqueEmail);
	uid = user.id;
...
test('Quick Create Modal - Open and Close', async ({ page }) => {
	...
	await test.step('Open modal with floating button', async () => {
		await page.click('button[title="Quick Add Event"]');
...
test('Quick Create Modal - NL Input Parsing', async ({ page }) => {
	...
	await test.step('Enter NL input', async () => {
		await page.fill('input[placeholder*="Try: Lunch"]', 'Lunch Friday at noon with John in Conference A');
		// Wait for parsing
	...
	await test.step('Manually fill required fields if not auto-filled', async () => {
		...
		const dateInput = page.locator('input[name="start"]');
...
test('Quick Create Modal - Description Contains Raw Input', async ({ page }) => {
	...
	await test.step('Enter NL input and manually set title/date if needed', async () => {
		await page.fill('input[placeholder*="Try: Lunch"]', 'Dinner tomorrow at 7 PM at my apartment');
		// Wait for parsing
		await page.waitForTimeout(3000);
